...
public class ProductoController {
	...
	public ModelAndView editarForm(Model model, @RequestParam String codigo, RedirectAttributes ra) {
		ProductoVO respuestaServicio = new ProductoVO();
		respuestaServicio.setMensaje("No se pudo cargar la vista de edición, intentar nuevamente.");
		try {
	...
	@GetMapping("/eliminar")
	public ModelAndView eliminar(Model model, @RequestParam String codigo, RedirectAttributes ra) {
		ProductoVO respuestaServicio = new ProductoVO();
		respuestaServicio.setMensaje("No se pudo eliminar al usuario, intentar nuevamente.");
		try {
			Producto eliminado = new Producto();
			eliminado.setCodigo(Integer.parseInt(codigo));
	...
	@PostMapping("/buscar")
	public String agregarSubmit(Model model, @RequestParam String nombre) {
		ProductoVO respuestaServicio = svc.findByNombre(nombre);
		model.addAttribute("mensaje", respuestaServicio.getMensaje());
